...
class Game3 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("spikes","./images/หนาม1.png");
        this.load.image("spikes2","./images/หนาม2.png");
        this.load.image("blockdoor","./images/bockdoor.png");
    }
    create() {
        ...
        bottuninteractive=this.physics.add.image(600,400,"bottuninteractive").setScale(0.3);
        ...
        blockdoor=this.physics.add.image(800,450,"blockdoor").setScale(1.5);

        spikesgroup=this.physics.add.group();
        spikesgroup2=this.physics.add.group();

        this.physics.add.collider(ground,playerMain);
        this.physics.add.collider(playerMain,blockdoor);
        ...
        this.physics.add.overlap(playerMain,bottuninteractive,()=>{
            blockdoor.destroy();
        });

        blockdoor.setImmovable();
        ground.setImmovable();
        playerMain.setCollideWorldBounds(true);
        bottuninteractive.setInteractive();
        ...
        eventFall=this.time.addEvent({
            delay:900,
            callback:function(){
                spikes=this.physics.add.image(300,600,"spikes",).setScale(0.15);
                spikes2=this.physics.add.image(600,0,"spikes2",).setScale(0.15);
                spikesgroup.add(spikes);
                spikesgroup2.add(spikes2);
                spikesgroup.setVelocityY(-600);
                spikesgroup2.setVelocityY(600)
            },
            callbackScope: this,
            loop: true,
            paused: false,
        });
        this.physics.add.overlap(playerMain,spikesgroup,()=>{
            this.scene.start("GameOver");
        });
        this.physics.add.overlap(playerMain,spikesgroup2,()=>{
            this.scene.start("GameOver");
        });
